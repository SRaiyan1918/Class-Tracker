...
// dpp and hw: 'yes' | 'no' | 'na'  (na = not provided by teacher)
// theory and notes: boolean (simple yes/no)
const EMPTY_FORM = {
              ...
              <div className="checkbox-grid">
                {STUDY_ITEMS.map(({ key, icon, label, triState }) => (
                  triState ? (
                        ...
                        {[
                          ...
                          <button
                            ...
                            style={{
                              ...
                              border: `2px solid ${
                                form[key] === val
                                  ? val === 'yes' ? '#10b981' : val === 'no' ? '#ef4444' : '#f59e0b'
                                  : 'var(--border)'
                              }`,
                              background: form[key] === val
                                ? val === 'yes' ? 'rgba(16,185,129,0.15)' : val === 'no' ? 'rgba(239,68,68,0.15)' : 'rgba(245,158,11,0.15)'
                                : 'transparent',
                              color: form[key] === val
                                ? val === 'yes' ? '#10b981' : val === 'no' ? '#ef4444' : '#f59e0b'
                                : 'var(--text-secondary)',
                    ...
                    /* ── Normal checkbox for Theory & Notes ── */
          ...
          <div className="class-card" key={cls.id}>
            ...
            <div className="study-work-grid">
              {STUDY_ITEMS.map(({ key, icon, label, triState }) => {
                ...
                let badge, cls2;
                if (triState) {
                  if (val === 'Yes')      { badge = '✅'; cls2 = 'completed'; }
                  else if (val === 'N/A') { badge = '⚠️ N/A'; cls2 = 'na'; }
                  else                    { badge = '❌'; cls2 = ''; }
                } else {
                  badge = val === 'Yes' ? '✅' : '❌';
                  cls2  = val === 'Yes' ? 'completed' : '';
                }
                return (
                  <div key={key} className={`study-item ${cls2}`}
                    style={cls2 === 'na' ? { opacity: 0.65, fontStyle: 'italic' } : {}}>
                    {icon} {label}: {badge}
                  </div>
                );
              })}
